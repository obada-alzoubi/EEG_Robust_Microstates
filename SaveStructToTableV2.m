function [success,err] = SaveStructToTableV2(dat,FileName,K, Delimiter)
    ...
    if fid == -1
        ...
        % Remove some fields
    metric = {'OrgTM', 'DeltaTM', 'ExpTM'};
    toRemove ={'Subject', 'Group', 'Condition'};
    c=4; 
    for i =1:K
        for m =1: length(metric)
            toRemove{c} = sprintf('%s_%d->%d', metric{m}, i, i);
            c = c +1 ;
        end
    end
    ...
    for i=1:length(toRemove)
        ind =find(strcmp( outnames, toRemove{i}));
        indices = [indices ind];
    end
    indices = setdiff([1:length(outnames)],indices);
    outnames = outnames(indices);
    outfields = outfields(:, indices);
