...
function com = pop_QuantMSTemplatesV2(ALLEEG, CURRENTSET, UseMeanTmpl, FitParameters, MeanSet, FileName)
...
%    MSStats(numel(SelectedSet)).DataSet = '';
    ...
    for s = 1:numel(SelectedSet)
        ...
        if UseMeanTmpl == false
            ...
            [MSClass,~,ExpVar] = AssignMStatesV2(ALLEEG(sIdx),Maps,par,ALLEEG(sIdx).msinfo.ClustPar.IgnorePolarity);
            if ~isempty(MSClass)
 %              MSStats = [MSStats; QuantifyMSDynamics(MSClass,ALLEEG(sIdx).msinfo,ALLEEG(sIdx).srate, DataInfo, '<<own>>')];
            ...
            ALLEEG(sIdx).msinfo.FitPar = par;
            [MSClass,~,ExpVar] = AssignMStatesV2(ALLEEG(sIdx),Maps,par, ALLEEG(MeanSet).msinfo.ClustPar.IgnorePolarity, ALLEEG(MeanSet).chanlocs);
            if ~isempty(MSClass)
%                MSStats = [MSStats; QuantifyMSDynamics(MSClass,ALLEEG(sIdx).msinfo,ALLEEG(sIdx).srate, DataInfo, ALLEEG(MeanSet).setname)]; 
    ...
    % Write CSV 
    SaveStructToTableV2(MSStats, csv_file,FitParameters.nClasses,',')
