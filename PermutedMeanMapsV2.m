function [MeanMap,SortedMaps,OldMapFit] = PermutedMeanMapsV2(in,RespectPolarity, Montage)
...
debug = false;
    ...
    while WorkToBeDone
        cnt = cnt + 1;
        if cnt >500
            fprintf('WE exit because of too many loops \n')
            break;
        end
        ...
        if debug == true
            ...
            for s = 1:nSubjects
                for c = 1:nMaps
                    subplot(5,nMaps,spidx);
                    [~, imap,~, ~]=dspCMap(squeeze(SortedMaps(s,c,:)),[X;Y;Z],'Resolution',5, 'Extrema');
                    %[imap] = OdspCMap(squeeze(SortedMaps(s,c,:)),[X;Y;Z],'Resolution',5);
                    %imap=imap';
%                     [~, m_max] = max(imap(:));
%                     [~, m_min] = min(imap(:));
%                     [I_max,J_max] = ind2sub(size(imap),m_max);
%                     [I_min,J_min] = ind2sub(size(imap),m_min); 
%                     hold on
%                     plot( [J_max; J_min],[I_max; I_min] ,'-ok', 'LineWidth',4,'MarkerSize',10)
%                     hold off
                    spidx = spidx+1;
                    if spidx >20; spidx = 1; end;
                end
            end
            for c = 1:nMaps
